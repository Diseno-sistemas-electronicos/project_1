...
SC_MODULE(pipeline_reg) {
	...
	sc_in<sc_uint<8> > of1In;	// Operando Fuente 1
	sc_in<sc_uint<8> > of2In;	// Operando Fuente 2
	sc_in<sc_uint<9> > odIn;		// Operando destino
	sc_out<sc_uint<4> > opcodeOut;	// OPCODE
	sc_out<sc_uint<8> > of1Out;	// Operando Fuente 1
	sc_out<sc_uint<8> > of2Out;	// Operando Fuente 2
	sc_out<sc_uint<9> > odOut;	// Operando destino

	//-- Method --//
	void pipeline_reg_routine(){
	while (true) {
		wait();
		if (enable.read() == 1) {
			if (write.read() == 0) {
